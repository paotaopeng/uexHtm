...
import com.wisemen.demo.callback.StringDialogCallback;
...
public class HttpsActivity extends BaseDetailActivity {
    ...
    public void btn_none_https_request(View view) {
        TaskWalker.get("https://www.baidu.com")
                .tag(this)
    ...
    public void btn_https_request(View view) {
        try {
            //自签名的证书需要在全局初始化的时候设置
            TaskWalker.get("https://kyfw.12306.cn/otn")
                    .tag(this)
                    .headers("Connection", "close")      //一部分自签名的https访问不成功，需要加上该控制头
                    .headers("header1", "headerValue1")
                    .params("param1", "paramValue1")
                    .execute(new StringDialogCallback(this) {
                        @Override
